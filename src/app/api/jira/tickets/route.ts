...
export async function GET(request: Request) {
    ...
    return NextResponse.json(
      {
        ok: false,
        error:
          "Paramètre scope requis (open | created | closed | sla_pec | sla_cloture).",
      },
      { status: 400 },
    );
  }
  ...
  if (scope === "created" && !weekId && year == null) {
    return NextResponse.json(
      { ok: false, error: "Pour scope=created, indiquez week ou year." },
      { status: 400 },
    );
  }

  if (
    (scope === "closed" || scope === "sla_pec" || scope === "sla_cloture") &&
    !weekId
  ) {
    return NextResponse.json(
      {
        ok: false,
        error: "Pour les scopes closed / SLA, indiquez week (ex. 2026-S32).",
      },
      { status: 400 },
    );
  }
